...
type ColumnType = ColumnDef<StudentProgressInterface, unknown>;

interface StudentProgressListPageProps {
...
export function StudentProgressListPage(props: StudentProgressListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StudentProgressInterface>>(
    () => `/student-progresses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StudentProgressInterface) => {
    if (hasAccess('student_progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/student-progresses/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/student-progresses/view/${record.id}`} passHref legacyBehavior>
            <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="student_progress" />
        </Flex>
