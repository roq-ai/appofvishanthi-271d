...
import { getContests, deleteContestById } from 'apiSdk/contests';
import { ContestInterface } from 'interfaces/contest';

type ColumnType = ColumnDef<ContestInterface, unknown>;
...
export function ContestListPage(props: ContestListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ContestInterface>>(
    () => `/contests?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ContestInterface) => {
    if (hasAccess('contest', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/contests/view/${row.id}`);
    }
  };
    ...
    { id: 'description', header: 'Description', accessorKey: 'description' },
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/contests/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('contest', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/contests/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('contest', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="contest" />
        </Flex>
        {hasAccess('contest', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/contests/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
